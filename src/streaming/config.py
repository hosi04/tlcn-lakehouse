...
# Throughput (pick one style):
# - EVENTS_PER_SECOND: target steady rate (~10/s => 100k events in ~2.8h)
# - PRODUCE_DELAY_MS: fixed pause per event; if > 0, overrides EVENTS_PER_SECOND
EVENTS_PER_SECOND = float(os.getenv("EVENTS_PER_SECOND", "10"))
PRODUCE_DELAY_MS = int(os.getenv("PRODUCE_DELAY_MS", "0"))

# PREVIEW_EVENTS=2 prints samples only; 0 pushes to Kafka
